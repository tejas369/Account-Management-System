...
stockaccount::stockaccount()	//constructor
{
	...
	set_cbal();
	cash_bal = get_cbal();
	//store the data from textfile into STL map data structure
...
void stockaccount::buy_update(string s, int n)		//function to update linklist
{
	...
	if (f == NULL)							//if symbol not found
	{
		...
		//addinf the new node to the end of the list if list not empty
		else
		{
...
void stockaccount::buyingshares()   //funcion to buy shares
{
	...
	set_cbal();
	cash_bal = get_cbal();			//get recent cash bal
	if (total_amt <= cash_bal)		//checking person have enough balance to buy shares
	{
		cash_bal = cash_bal - total_amt;
		...
		string str(buffer);
		//store transaction
		stock_his.open("stock_transaction_history.txt", ios::app);
		stock_his << left << setw(8) << "Buy";
		stock_his << left << setw(11) << stocksymb;
		stock_his << left << setw(11) << NOS;
		stock_his << left << setw(13) << s_price;
		stock_his << left << setw(14) << total_amt;
		stock_his << left << setw(19) << str << endl;
		stock_his.close();
		//store bank transaction
		bank_his.open("bank_transaction_history.txt", ios::app);
		bank_his << left << setw(20) << "withdraw(stock)";
		bank_his << left << setw(15) << total_amt;
	...
	sort();
	save_port_val();  //save portfolio val
}
...
void stockaccount::sellingshares()			//function to sell shares
{
	...
	set_cbal();
	cash_bal = get_cbal();		//read recent cash balance
	total_amt = NOS*s_price;	
	cash_bal = cash_bal + total_amt;		//update cash balance
	...
	bank_his.open("bank_transaction_history.txt", ios::app);
	bank_his << left << setw(20) << "deposit(stock)";
	bank_his << left << setw(15) << total_amt;
	bank_his << left << setw(25) << str;
	bank_his << left << setw(10) << cash_bal << endl;
	bank_his.close();
	...
	sort();
	save_port_val();	//save portfolio value
...
void stockaccount::read_port()
{
	...
	int share_temp;
	ifstream myfile6;
	myfile6.open("portfolio.txt", ios_base::in);
	if (myfile6.is_open())
	{
		while (!myfile6.eof())	//till end of line
		{
			getline(myfile6, line1);
			B << line1;             
...
void stockaccount::save_port_val()		//save total potfolio value over period of time
{
	...
	if (size == 0)
		port_val = cash_bal; // if at exit, size portfolio is empty, then store portfolio value as CashBal.
